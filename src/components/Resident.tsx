...
export const Resident = ({
  ...
  const handleIsFriendsState = (isFriends: boolean) => {
    setIsFriends(isFriends);
  };
  //console.log("ohot " + photo);
  //console.log("photo undefnien " + (photo == ""));
  ...
  const fetchData = async () => {
    ...
    if (residentId == id) {
      ...
      if(resident?.snapchat) socials.push({platform: "snapchat", username: resident.snapchat})
        if(resident?.x) socials.push({platform: "x", username: resident.x})
          if(resident?.instagram) socials.push({platform: "instagram", username: resident.instagram})
            if(resident?.facebook) socials.push({platform: "facebook", username: resident.facebook})
  ...
  return (
    ...
    >
      ...
      <Image
        ...
        source={{
          ...
          uri: resident?.backgroundPhoto
            ? `data:image/jpeg;base64,${resident?.backgroundPhoto}`
            : ``,
        }}
      />
      ...
      {resident?.hometown && <Text style={styles.hometown}>{resident?.hometown}</Text>}
      {resident?.age && <Text style={styles.age}>{resident?.age}</Text>}
      {resident?.biography && <Text style={styles.bio}>{resident?.biography}</Text>}
...
data={socialMedia}
renderItem={({item}) => <SocialMedia platform={item.platform} username={item.username} />}
keyExtractor={item => item.platform}
/>

      {!isConnected && (residentId != id) && (
